...
PosixEndpointImpl::PosixEndpointImpl(EventHandle* handle,
  ...
  if (peer_address.ok()) {
    ...
    // sizeof(sa_family_t)), yielding a truncated URI like "unix:". Fall back to
    // the local address (the server's socket path) so that
    // ServerContext::peer() returns a meaningful value instead.
