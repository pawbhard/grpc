...
// in each direction is compressed by the sending side's filter and
// transparently decompressed by the receiving side's filter, so each endpoint
// sees the original bytes back -- but flagged WAS_COMPRESSED, proving it really
// went over the wire compressed rather than passing through untouched.
