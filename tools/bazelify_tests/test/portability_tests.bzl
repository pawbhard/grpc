...
def generate_run_tests_portability_tests(name):
    ...
    for language in ["c", "c++"]:
        compiler_configs = [
            ["gcc_10", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=17 --cmake_configure_extra_args=-DgRPC_BUILD_TESTS=OFF", "tools/dockerfile/test/cxx_gcc_10_x64.current_version"],
            ...
            ["clang_19_cxx23", "--cmake_configure_extra_args=-DCMAKE_CXX_STANDARD=23 --cmake_configure_extra_args=-DCMAKE_C_COMPILER=clang --cmake_configure_extra_args=-DCMAKE_CXX_COMPILER=clang++ --cmake_configure_extra_args=-DCMAKE_CXX_FLAGS=-Wno-deprecated-declarations", "tools/dockerfile/test/cxx_clang_19_x64.current_version"],
        ]
